        ...
        <?php
            ...
            if (isset($_COOKIE["admin_user"]) && isset($_COOKIE["admin_uid"]) && isset($_GET["company_id"])) {
                ...
                if ($admin_id>0) {
                    ...
                    $list_categories=$acc->getCategories($_GET["company_id"]);
                }else{
...
<nav class="w3-sidebar w3-animate-left w3-red w3-collapse w3-top w3-large w3-padding" style="z-index:3;width:300px;font-weight:bold;" id="mySidebar"><br>
  ...
  <div class="w3-bar-block">
      ...
      <a href="company_service.php?company_id=<?php echo $_GET["company_id"]; ?>" onclick="w3_close()" class="w3-bar-item w3-button w3-hover-white">Back to service</a>
  ...
  <div class="w3-container" style="margin-top:80px" id="showcase">
    ...
    <form id="service-form" method="POST">
        ...
        <div class="w3-section">
            ...
            <input placeholder="Service name" class="w3-input w3-round-xlarge w3-border" type="text" name="servicename" required="">
        </div>
        <div class="w3-section">
            <label>Description</label>
            <input placeholder="Description" class="w3-input w3-round-xlarge w3-border" type="text" name="description" required="">
        </div>
        <div class="w3-section">
            <label>Price</label>
            <input placeholder="Price" class="w3-input w3-round-xlarge w3-border" type="number" name="price" required="">
        </div>
        <div class="w3-section">
            <label>Categories</label>
            <select class="w3-input w3-round-xlarge w3-border" type="text" name="category_id" required="">
                <option value="">Please select an item</option>
                <!--For loop goes here...-->
                <?php 
                for($i=0;$i<count($list_categories);$i++){
                    ?><option value="<?php echo $list_categories[$i]->CategoryID; ?>"><?php echo $list_categories[$i]->CategoryName; ?></option><?php
                }
                ?>
            </select>
        </div>
        <button type="button" onclick="admin.services.addService(this)" class="w3-button w3-round-large w3-block w3-padding-large w3-red w3-margin-bottom">Add new service</button>
    </form>
