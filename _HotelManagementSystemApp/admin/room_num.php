        ...
        <?php
            ...
            if (isset($_COOKIE["admin_user"]) && isset($_COOKIE["admin_uid"]) && isset($_GET["company_id"])) {
                ...
                if ($admin_id>0) {
                    ...
                    $list_room_type=$acc->getRoomTypes($_GET["company_id"]);
                }else{
...
<nav class="w3-sidebar w3-animate-left w3-red w3-collapse w3-top w3-large w3-padding" style="z-index:3;width:300px;font-weight:bold;" id="mySidebar"><br>
  ...
  <div class="w3-bar-block">
      ...
      <a href="room_type.php?company_id=<?php echo $_GET["company_id"]; ?>" onclick="w3_close()" class="w3-bar-item w3-button w3-hover-white">Add room type</a>
...
<div class="w3-main" style="margin-left:340px;margin-right:40px;min-height: 400px;">
  ...
  <div class="w3-container" style="margin-top:80px" id="showcase">
    ...
    <?php 
    if(count($list_room_type)>0){
        ?>
           <form id="room-num-form" method="POST">
               <input type="hidden" value="<?php echo $_GET["company_id"]; ?>" name="company_id"/>
        <div class="w3-section">
            <label>Room number:</label>
            <input placeholder="Room number" class="w3-input w3-round-xlarge w3-border" type="number" name="roomnum" required="">
        </div>
        <div class="w3-section">
            <label>Room type:</label>
            <select placeholder="Room type" class="w3-input w3-round-xlarge w3-border" type="text" name="roomtype" required="">
                <option value="">Please select room type</option>
                <!--For loop goes here...-->
                <?php for($index=0;$index<count($list_room_type);$index++){
                    ?><option value="<?php echo $list_room_type[$index]->RoomTypeID; ?>"><?php echo $list_room_type[$index]->RoomName; ?></option><?php
                } ?>
            </select>
        </div>
               <button type="button" onclick="admin.room.addRoomNum(this)" class="w3-button w3-round-large w3-block w3-padding-large w3-red w3-margin-bottom">Add room number</button>
    </form> <?php
    } ?>
